...
export class ASTBuilder {
  ...
  parseStatement() {
    ...
    if (token.type === TokenType.keyword) {
      if (token.value === 'make') {
        ...
        return new VariableDeclaration(idToken.value, expr, { line: token.line, column: token.column });
      } else if (token.value === 'assign') {
        const idToken = this.tokens[this.index++];
        if (idToken.type !== TokenType.identifier) throw new Error(`Expected identifier after 'assign'`);
        const expr = this.parseFullExpression();
        return new AssignmentExpression(idToken.value, expr, { line: token.line, column: token.column });
      } else if (token.value === 'print') {
        const expr = this.parseFullExpression();
        return new PrintStatement(expr, { line: token.line, column: token.column });
      } else if (token.value === 'if') {
        const test = this.parseFullExpression();
        const consequent = this.parseBlock('endif');
        return new IfStatement(test, consequent, { line: token.line, column: token.column });
      } else if (token.value === 'as') {
        const test = this.parseFullExpression();
        const body = this.parseBlock('repeat');
        return new WhileStatement(test, body, { line: token.line, column: token.column });
      } else {
      ...
      const expr = this.parseFullExpression();
      return new AssignmentExpression(token.value, expr, { line: token.line, column: token.column });
    } else {
  ...
  parseFullExpression() {
    ...
    while (this.index < this.tokens.length && this.tokens[this.index].type === TokenType.operator) {
      ...
      expr = new BinaryExpression(expr, opToken.value, right, expr.loc);
    }
    return expr;
  }
  ...
  parseExpression(token) {
    if (token.type === TokenType.identifier) {
      return new Identifier(token.value, { line: token.line, column: token.column });
    } else if (token.type === TokenType.number) {
      return new NumericLiteral(Number(token.value), { line: token.line, column: token.column });
    } else if (token.type === TokenType.string) {
      return new StringLiteral(token.value.slice(1, -1), { line: token.line, column: token.column }); // Remove quotes
    } else {
