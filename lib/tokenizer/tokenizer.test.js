...
describe('Tokenizer', () => {
  ...
  it('identifier', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.identifier, 'test')]);
  ...
  it('keyword', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.identifier, 'make')]);
    assert.strictEqual(tokens.length, 1);
  ...
  it('operator', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.operator, '*')]);
    assert.strictEqual(tokens.length, 1);
  ...
  it('number', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.number, '42')]);
    assert.strictEqual(tokens.length, 1);
  ...
  it('invalid number', async () => {
    try {
      await testStream([new PreprocessingToken(PreprocessingTokenType.number, '4a4')]);
      assert.fail('Expected exception');
    } catch (e) {
      assert(e instanceof TokenizerError);
  ...
  it('string', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.string, "'test'")]);
    assert.strictEqual(tokens.length, 1);
    assert.strictEqual(tokens[0].value, 'test');
  ...
  it('another keyword', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.identifier, 'if')]);
    assert.strictEqual(tokens.length, 1);
  ...
  it('ignores whitespace', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.whitespace, '')]);
    assert.strictEqual(tokens.length, 0);
  });

  it('ignores linefeed', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.linefeed, '')]);
    assert.strictEqual(tokens.length, 0);
  });

  it('ignores eof', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.eof, '')]);
    assert.strictEqual(tokens.length, 0);
  });

  it('ignores unknown', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.unknown, '@')]);
    assert.strictEqual(tokens.length, 0);
  });
  ...
  it('string with spaces', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.string, "'hello world'")]);
    assert.strictEqual(tokens.length, 1);
  ...
  it('large number', async () => {
    const tokens = await testStream([new PreprocessingToken(PreprocessingTokenType.number, '123456789')]);
    assert.strictEqual(tokens.length, 1);
...
describe('Token', () => {
  it('toString', () => {
    const token = new Token(TokenType.identifier, 'x', 1, 5);
    assert.strictEqual(token.toString(), 'identifier x');
  });
});
