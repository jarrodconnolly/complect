...
export class BabelAST extends Transform {
  ...
  #tokenToBabelValue(token) {
    ...
    if (token.type === TokenType.identifier) {
      ...
      throw new ASTError(`Unsupported token type for value: ${token.type} at line ${token.line}, column ${token.column}`);
    }
    node.loc = {
  ...
  #handleVariableDeclaration() {
    ...
    if (identifierToken.type !== TokenType.identifier) {
      this.destroy(new ASTError(`Invalid variableDeclaration. Expected identifier at line ${identifierToken.line}, column ${identifierToken.column}.`));
    }
    if (literalToken.type !== TokenType.string && literalToken.type !== TokenType.number) {
      this.destroy(new ASTError(`Invalid variableDeclaration. Expected number or string at line ${literalToken.line}, column ${literalToken.column}.`));
    }
  ...
  #handleVariableAssignment() {
    ...
    const literalToken = this.#currentTokens[2];
    if (identifierToken.type !== TokenType.identifier) {
      this.destroy(new ASTError(`Invalid variableAssignment. Expected identifier at line ${identifierToken.line}, column ${identifierToken.column}.`));
    }
    if (literalToken.type !== TokenType.string
      && literalToken.type !== TokenType.number
      && literalToken.type !== TokenType.identifier) {
      this.destroy(new ASTError(`Invalid variableAssignment. Expected number, string, or identifier at line ${literalToken.line}, column ${literalToken.column}.`));
    }
  ...
  #handleAssignmentExpression() {
    ...
    identifier.loc = {
      ...
      end: { line: assignIdentifierToken.line, column: assignIdentifierToken.column + assignIdentifierToken.value.length - 1 }
    };
