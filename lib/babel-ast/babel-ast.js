...
export class BabelAST {
  ...
  #handleVariableDeclaration() {
    ...
    if (identifierToken.type !== TokenType.identifier) {
      throw new ASTError(`Invalid variableDeclaration. Expected identifier at line ${identifierToken.line}, column ${identifierToken.column}.`);
    }
    if (literalToken.type !== TokenType.string && literalToken.type !== TokenType.number) {
      throw new ASTError(`Invalid variableDeclaration. Expected number or string at line ${literalToken.line}, column ${literalToken.column}.`);
    }
  ...
  #handleVariableAssignment() {
    ...
    const literalToken = this.#currentTokens[2];
    if (identifierToken.type !== TokenType.identifier) {
      throw new ASTError(`Invalid variableAssignment. Expected identifier at line ${identifierToken.line}, column ${identifierToken.column}.`);
    }
